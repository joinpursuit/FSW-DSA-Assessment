...
function inputSum(arr, targetInt) {
  ...
  let result = false;
  arr.forEach(e => {
    if (pairs[targetInt - e]) result = true;
    else pairs[e] = 1;
  });
  return result;
}
...
function recursiveSum(input) {
  if (input.length === 0) return 0;
    ...
    return (
      input[input.length - 1] + recursiveSum(input.slice(0, input.length - 1))
    );
  }
}
...
function maxDepth(tree) {
  if (!tree) return 0;
  else return 1 + Math.max(maxDepth(tree.left), maxDepth(tree.right));
}
